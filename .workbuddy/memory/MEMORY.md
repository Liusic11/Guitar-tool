...
- 音阶训练器（CAGED 5 形状 / 七声 7 位置，按琴颈从 0 品升序；地图/跟拍/模进；演示边播边高亮）：✅
...
- 耳朵训练独立模块（Ear Trainer）：✅ 顶栏第 4 视图「耳朵」。`src/components/EarTrainer.tsx` 含音程辨识（12 选项+曲例锚点）/ 和弦辨识（9 类型）/ 音阶听辨（7 音阶）；复用 `audioEngine.pluck/strum`，选项随机打乱，判定后高亮+解析+连对统计；空格再听、回车下一题。
- 节奏预设扩充：✅ `rhythm.ts` 的 `RHYTHM_PRESETS` 增至 11 种（含 Bossa / Samba / Reggae 一键掉 / 16分直线 / 半拍摇滚 等鼓点），练习 scale/chord 时律动选择更丰富。
- Jam 模块（第 5 视图）：✅ `src/lib/progressions.ts` + `src/components/JamTrainer.tsx`。6 个进行预设（1645/1564/4536/12小节蓝调/ii–V–I/Andalusian），全局父音阶或每和弦换音阶，进行时间轴+大指板（和弦形状↔音阶音双态），backing 出声开关，底部 RhythmBar 跟拍推进。
- 乐句库（第 6 视图「乐句」）：✅ 2026-08-15。`src/lib/licks.ts` 20 条手写乐句（蓝调/funk/爵士/摇滚各 5，A 参考根音、12 根音可移调、移调后度数不变）+ `src/components/LickLibrary.tsx`（指板高亮+逐音播放+逐音清单「弦·品·音名·级数」+老师拆解）+ Jam 页每和弦乐句建议（`licksForChordType`）。补上「词汇→造句」中间层。
- CAGED G/C 形补全：✅ 2026-08-15。`chords.ts` `listChordVoicings` 从 4 候选 → 6 候选（+G 形 6弦伸展 / C 形 5弦伸展）：偏移模板 + 就近取和弦音，七和弦保 7 音、色彩和弦保色彩音；ChordChanges 锁把位可选 G/C 形。

## 待建（方向：确定性、规则可算的练习，不做 LLM agent / 自动拆歌等「智能过头」功能）
- 和弦识记训练（主动回忆和弦形状）——优先级下调：用户已会开放和弦，此模块非刚需。
- ~~节奏律动(扫弦)独立页（#40）：2026-08-13 先做后删——用户认为 scale/chord 练习已含节奏，独立扫弦页没必要；改为扩充 drum 预设~~
- ~~乐理贯通层（WHY）：已在 2026-08-11 完成（harmony.ts + session.ts + 两页贯通面板）~~
- ~~模进训练（Patterns）：已在 2026-08-11 完成~~
- ~~歌曲拆解 / Song Workout（自动拆歌为智能过头功能，已砍）~~
- ~~耳朵训练独立模块：已在 2026-08-13 完成（EarTrainer）~~
- ~~Jam 伴侣：已在 2026-08-13 完成（JamTrainer，第 5 视图）~~
- ~~乐句库（造句层）：已在 2026-08-15 完成（LickLibrary，第 6 视图；乐句可继续往 licks.ts 里加）~~
- 练习路线/打卡页（更后期，五模块+乐句齐了之后的收口层）
